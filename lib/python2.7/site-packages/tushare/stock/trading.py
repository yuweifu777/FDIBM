...
def _today_ticks(symbol, tdate, pageNo, retry_count, pause):
    ...
    for _ in range(retry_count):
        time.sleep(pause)
        try:
            print(ct.TODAY_TICKS_URL % (ct.P_TYPE['http'],
...
def bar(code, start_date=None, end_date=None, ktype='D', asset='E', 
           adj = None,
           ma = [],
           volratio = False,
           retry_count = 3):
    ...
    start_date:开始日期
    end_date:结束日期
    ktype:支持1/5/15/30/60分钟,周/月/季/年
    asset:证券类型,E:股票和交易所基金，INDEX,F:期货,O:期权
    adj:复权类型,None不复权,qfq:前复权,hfq:后复权
    ma:均线,支持自定义均线频度，如：ma5/ma10/ma20/ma60/maN
    volratio:量比,默认为False
    retry_count:网络重试次数
    
    Return
    ----------
    DataFrame
    code/open/close/high/low/vol成交量/amount成交额/maN均价/vol_ratio量比
    """
    for _ in range(retry_count):
        try:
            api = TdxHq_API()
            ...
            ktype = ktype.strip().upper()
            asset = asset.strip().upper()
            func = getattr(api, ct.ASSET[asset])
            if ktype in ct.KTYPE_LOW_COLS:
                data = pd.concat([api.to_df(func(ct.KTYPE[ktype], 
                                  mkcode, code, (9 - i) * 800, 800)) 
                                  for i in range(10)], axis=0)
                data['datetime'] = data['datetime'].apply(lambda x: str(x[0:10]))
            if ktype in ct.KTYPE_ARR:
                data=[]
                for i in range(50): # range for python2/3
                    data += func(ct.KTYPE[ktype], mkcode, code, (49-i) * 800, 800)
                data = api.to_df(data)
            ...
            if adj is not None:
                ...
                if ktype in ct.KTYPE_LOW_COLS: 
                    ...
                    data['adj_factor'] = data.index.map(lambda x: df.ix[x.strftime('%Y-%m-%d')]['adj_factor'])
            ...
            if ma is not None and len(ma) > 0:
                for a in ma:
                    if isinstance(a, int):
                        data['MA%s'%a] = MA(data['close'], a).map(ct.FORMAT)
            data = data.sort_index(ascending=False)
            if volratio:
                data['vol5'] = MA(data['vol'], 5)
                data['mean'] = data['vol5'].shift(-5)
                data['vol_ratio'] = (data['vol'] / data['mean']).map(ct.FORMAT)
